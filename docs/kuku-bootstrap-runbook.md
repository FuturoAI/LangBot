...
- rewrites `web/.env` with `NEXT_PUBLIC_API_BASE_URL`, preferring `data/config.yaml` `api.port` and falling back to `http://127.0.0.1:5300`. If you want a one-off override, run `LANG_BOT_HTTP_PORT=15300 ./scripts/kuku-bootstrap.sh` so `curl` and the web UI target the same port.
